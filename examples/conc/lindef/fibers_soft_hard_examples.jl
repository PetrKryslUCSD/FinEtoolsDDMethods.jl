module fibres_soft_hard_examples
using FinEtools
...
function _execute(kind, nelperpart, nbf1max, ref, mesh, boundary_rule, interior_rule, make_femm)
    # Isotropic material
    Em = 1.0
    # num = 0.4999 
    num = 0.3 
    ...
    femm = FEMMBase(IntegDomain(subset(fes, fiberel), interior_rule))
...
function test(; kind = "tet", nelperpart = 200, nbf1max = 5, ref = 1)
    ...
    _execute(kind, nelperpart, nbf1max, ref, mesh, boundary_rule, interior_rule, make_femm)
end
