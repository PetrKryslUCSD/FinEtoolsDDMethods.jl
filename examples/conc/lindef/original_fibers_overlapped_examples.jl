module original_fibers_overlapped_examples
using FinEtools
...
function _execute(label, kind, Em, num, Ef, nuf, nelperpart, nbf1max, nfpartitions, overlap, ref, mesh, boundary_rule, interior_rule, make_femm, itmax, visualize)
    ...
    K = nothing
    GC.gc()
    @info("Global stiffness matrix: $(mebibytes(K_ff)) [MiB]")
    ...
    @info "Total partition allocations: $(sum([mebibytes(_p.factor) for _p in partitions])) [MiB]" 
    ...
    (u_f, stats) = pcg_seq((q, p) -> mul!(q, K_ff, p), F_f, zeros(size(F_f));
        (M!)=(q, p) -> M!(q, p),
        itmax=itmax, atol=0.0, rtol=1e-6, normtype = KSP_NORM_UNPRECONDITIONED)
    t1 = time()
...
function test(label = "soft_hard"; kind = "hex", 
    Em = 1.0e3, num = 0.4999, Ef = 1.0e5, nuf = 0.3, 
    nelperpart = 200, nbf1max = 5, nfpartitions = 2, overlap = 1, ref = 1, visualize = false)
    ...
    itmax = 2000
    _execute(label, kind, Em, num, Ef, nuf, 
        nelperpart, nbf1max, nfpartitions, overlap, ref, 
        mesh, boundary_rule, interior_rule, make_femm, itmax, visualize)
end
