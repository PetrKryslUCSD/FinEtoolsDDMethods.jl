...
Version: 08/16/2024
"""
module Poisson2D_overlapped_3_examples
using FinEtools
...
function _execute(N, mesher, volrule, nelperpart, nbf1max, nfpartitions, overlap, itmax, relrestol, visualize)
    ...
    partitions = [PartitionCoNCDDMPIModule.CoNCPartitionData(cpi, i, fes, Phi, make_matrix)
    ...
    t0 = time()
    norm_F_f = norm(F_f) 
    (u_f, stats) = pcg_seq((q, p) -> PartitionCoNCDDMPIModule.partition_multiply!(q, partitions, p), F_f, zeros(size(F_f));
        (M!)=(q, p) -> PartitionCoNCDDMPIModule.precondition_solve!(q, Krfactor, Phi_f, partitions, p),
        itmax=itmax, atol=relrestol * norm_F_f, rtol=0)
    @info("Number of iterations:  $(stats.niter)")
    stats = (niter = stats.niter, residuals = stats.residuals ./ norm_F_f)
    scattersysvec!(Temp, u_f, DOF_KIND_FREE)
